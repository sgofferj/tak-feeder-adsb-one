...
const myCallsign = (typeof process.env.CALLSIGN !== 'undefined') ? process.env.CALLSIGN : "adsb.one";
...
const cotDBFile = (typeof process.env.COTDB !== 'undefined') ? process.env.COTDB : null;
const countriesDBFile = (typeof process.env.COUNTRIESDB !== 'undefined') ? process.env.COUNTRIESDB : null;
const LOGUNK = (typeof process.env.LOGUNK !== 'undefined') ? (process.env.LOGUNK == "true") : false;
...
function getCOT(item) {
    ...
    if (cotdb != null) {
        if (typeof cotdb[hexid] !== 'undefined') cotdata = cotdb[hexid];
            ...
            if (LOGUNK === true) console.log(`"${hexid}","COT","${reg}","${model}","${affil["country"]}"`)
        }
    }
...
module.exports.adsb2cot = (item) => {
    ...
    if ((item.hasOwnProperty('lat')) && (item.hasOwnProperty('lon')) && ((item.hasOwnProperty('alt_geom')) || (item.hasOwnProperty('alt_baro')))) {
        ...
        let actype;
        if (cotdata[2]) actype = cotdata[2];
        else actype = (item.hasOwnProperty('t')) ? item.t : 'unknown';
        let reg = cotdata[1];
        ...
        let callsign = (typeof item.callsign !== 'undefined') ? item.callsign : "ICAO-" + hex;

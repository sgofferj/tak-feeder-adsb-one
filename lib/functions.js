...
module.exports.adsb2cot = (item) => {
  ...
  if (
    ...
    let alt = item.hasOwnProperty("alt_geom")
      ? parseFloat(item.alt_geom)
      : parseFloat(item.alt_baro);
    if (isNaN(alt)) alt = 0;
    ...
    if (item.type == "mode_s" || item.type == "mlat") {
      packet["event"]["detail"]["__milicon"] = {
        _attributes: { id: SIDC },
        unitmodifier: [
          { _attributes: { code: "G" }, _text: item.type.toUpperCase() },
        ],
      };
    } else {
      packet["event"]["detail"]["__milicon"] = {
        _attributes: { id: SIDC },
        unitmodifier: [{ _attributes: { code: "G" }, _text: "" }],
      };
    }
